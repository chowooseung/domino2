...
class Rig(component.Rig):
    ...
    def rig(self):
        ...
        for i, m in enumerate(self["guide_matrix"]["value"]):
            ...
            cmds.setAttr(f"{ctl}.sx", channelBox=True)
            cmds.setAttr(f"{ctl}.sy", channelBox=True)
            cmds.setAttr(f"{ctl}.sz", channelBox=True)
            ...
            cmds.setAttr(f"{npo_loc}.s", lock=False)
            cmds.setAttr(f"{ctl_loc}.s", lock=False)
